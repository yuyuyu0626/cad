...
class WeightedHeatmapMSELoss(nn.Module):
    ...
    def forward(
        ...
        sq_error = (pred_heatmaps - target_heatmaps) ** 2
        if heatmap_mask is None:
            per_channel = sq_error.mean(dim=(-2, -1))
        else:
            spatial = heatmap_mask.float()
            denom_spatial = spatial.sum(dim=(-2, -1)).clamp_min(1.0)
            per_channel = (sq_error * spatial).sum(dim=(-2, -1)) / denom_spatial
